...
- All 54 brand resources are listed in `brand-assets.json` with SHA-256 hashes; screenshots are retained separately in `assets/screenshots/` and unchanged. Inventory filenames are relative to `assets/brand/`.
...
- Every manifest image reference resolves within `assets/brand/`; relative Markdown image/document links resolve across the shared assets and handoff directories.
- The transfer ZIP preserves `handoffs/mireqo-webpage/` and `assets/` at the archive root. Its CRCs and contents match those repository directories; the spec alone does not include its referenced assets.
