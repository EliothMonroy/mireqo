...
This document records the agreed architecture for Mireqo's Android and iOS MVP. The Expo application, pnpm workspace, backend API/worker foundation, and Discover area browsing, date/category filters and collection lists are implemented. Discover uses shared area/event contracts, a local-development PostgreSQL demo catalog, TanStack Query session data and an Expo SQLite area preference. Discovery uses backend-issued time contexts and independent section queries. Event Details and local Saved Events are implemented; live ingestion and later product stages remain pending. See [stage-2 decisions](plans/discover-collections/decisions.md).
...
- The API queries the stored catalog. Implemented endpoints are `/v1/areas` and `/v1/events?areaId=...&limit=...&cursor=...` for exact browse-area membership. Pagination uses local schedule-date/time keys with stable event ID, bound to area and demo dataset version; date-only sorts before exact times on that date and unannounced last. Additive `/v1/discovery/context` and `/v1/discovery/events` implement date/category/collection filtering; legacy `/v1/events` retains its all-demo-dates shape and behavior. Additive `/v1/events/:eventId` supplies global-ID details independently of discovery eligibility; Search, geographic lookup and later ranking remain pending.
...
Known end times classify an event as Past at end equality. With no end time, Past begins after the event-local day and is labeled Date passed; undated events stay in a separate Upcoming group. Cancellation/postponement remains independent. Multi-session identity is still deferred. Do not invent missing times.
...
## Discover date and collection flow

The implemented stage-2 flow uses Today, Tomorrow, This Weekend and This Week in the selected area’s calendar. Default eligibility excludes exact starts before the browsing reference instant, includes date-only events through their full day, and places unannounced dates last. Explicit date filters include their complete local days and exclude unannounced schedules. Temporal collections appear only under the default date choice; Free Events and Music combine with the selected date. General results and the four initial categories remain reachable.

The backend issues an opaque signed context containing area/dataset identity, effective instant, local date and a 24-hour expiry. Section previews and full-list pages share that token. Cursors bind the complete filter context and a boundary row inside the filtered SQL result. API restarts, dataset changes and expiry require a fresh context. No migration is needed for the current JSONB-backed predicates. These process-local signing semantics apply to the single-process local demo API, not a production multi-instance deployment contract.

A session provider owns the Discover date and area context. Manual refresh, app foreground and selected-area local midnight request a fresh server context. Date/filter changes reset incompatible data identities; changing area retains the date choice. Independent section queries retain same-scope content with a stale message if a newer generation fails. Category and collection lists are native stack routes carrying validated identifiers, not event objects; the parent stays mounted with its position and date intact. Each full list begins at the first event, not the preview continuation. Stage 2 persisted only area preference; stage 3 adds saved membership and snapshots as described below.

## Event Details and Saved Events

Stage 3 implements Discover and Saved tabs, with the Discover index and existing `/events` URL in its own preserved stack. `/event?id=...` is shared above tabs; only a validated stable ID travels in the route. Invalid/direct entry has a Discover fallback. Search and Settings remain deferred.

`AppProviders` composes one reactive saved store backed by namespaced SQLite tables in `mireqo.db`; preference tables remain separate and intact. Serialized atomic writes expose optimistic changes to every subscriber, track per-event intent revisions, and reconcile a latest failed write against confirmed persisted state with retry. Detail refresh cannot create membership or overwrite a newer membership intent. Snapshot validation uses shared contracts. Summary-only snapshots carry capture time; successful details carry their own refresh time. Errors keep prior snapshots and freshness.

The additive details envelope contains the unchanged summary plus area/demo provenance and nullable end instant, description, address and external URL. Migration004 adds nullable JSONB without modifying applied migrations or legacy summary shapes. Explicit v3 demo seed preserves all198 IDs and adds synthetic details; no worker/live source behavior changes. Query identity is event ID and each fetched response verifies identity. Native Share and HTTPS Linking are behind data adapters. Demo URLs only open example.org, labeled Open demo page; location lookup requires venue and address.

Saved hydrates without remote area/context dependencies, defaults Upcoming with undated group, and presents Past in reverse chronological order. Foreground and a one-second clock refresh temporal labels at event boundaries. Manual saved refresh processes only current saved IDs serially and guards each response with a membership revision. Saved does not synchronize to the backend or download images.
