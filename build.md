...
### Event Details and Saved Events development

Stage 3 adds `GET /v1/events/:eventId`, with colon-containing demo IDs URL-encoded, for example `/v1/events/demo%3Acoacalco%3A01`. Apply migration004 explicitly with `pnpm db:migrate` before API startup; readiness checks its new nullable details column. Existing rows remain readable with null details. Explicit `pnpm db:seed:demo 2026-09-18` supplies v3 synthetic metadata while preserving all198 IDs; reads never seed. Old cursors invalidate on deliberate v3 reseed. No dependency, lockfile or native configuration change requires a shell rebuild.

The mobile app now includes Discover and Saved tabs and `/event?id=...`. Saved uses namespaced `mireqo_saved_migrations` and `mireqo_saved_events` tables alongside selected-area preferences in `mireqo.db`. Core host checks cover temporal rules, saved write ordering/failure/retry, stale enrichment, detail/offline screens, control independence and safe native-action adapters. Real SQL integration covers additive migration preservation and all198 detail responses, strict legacy compatibility and404/400/503 boundaries. Native persistence, tab/back behavior, share sheets and external launch remain separate checks recorded in [stage3 implementation](plans/event-details-saved/implementation.md).

Archived helper handoff documents are excluded from formatting so historical evidence remains unchanged; all current source and feature records remain checked.
